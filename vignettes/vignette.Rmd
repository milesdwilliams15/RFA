...
RFR provides an alternative to mathching for generating an ATE from observational data. RFR uses a RF model fitted to the response and treatment variable respectively. Each set of fitted values is estimated as a function of covariates $x_{ip} \in X_i$ where $X_i \not\!\perp\!\!\!\perp y_i$ and $X_i \not\!\perp\!\!\!\perp z_i$. The variables $y_i$ and $z_i$ denote vectors of response and treatment values for the $i^\text{th}$ observation where $i \in I = \{1,2,...,n \}$. 
...
RFR's solution to this problem is to partial out the variation in $y_i$ and $z_i$ explained by $X_i$ prior to estimating the ATE:
...
`rfr` takes a formula object---**where the first right-hand side variable is assumed to be the treatment and the remaining covariates confounders**---and the data frame containing the variables as inputs and returns a list containing the estimated ATE:
...
From the above, we see that the RFR routine estimates the ATE of a GOT phone call on probability of turning out to vote is aproximately `r round(result$ate,3)`. Is this ATE statistically significant? To find out, I can use the `summary.rfr` function to return the summary statistics for the calculated ATE:
...
We can also use the `plot.rfr` function to plot the distribution of bootstrapped ATE replicates:
