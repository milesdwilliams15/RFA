...
The latest version of the `RFA` package includes two additional functions that help in performing these diagnostics. These are `get_cod()` and `get_importance()`.
...
Inference for each of the COD estimates is done via bootstrapping with weighted resampling, where the probability of a given observation being sampled per iteration is equal to $w_i / \sum_iw_i$.
